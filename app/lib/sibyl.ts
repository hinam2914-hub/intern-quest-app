...
export function getEducationSibyl(education: string): { cog: number; grit: number } {
    ...
    const e = education.replace(/\s/g, "");
    // 私立文系ベースの6段階。国公立は来る想定が薄いのでティア2相当に一括
    if (/早稲田|慶應|慶応|上智|ICU|国際基督教/.test(e)) return { cog: 15, grit: 8 };
    if (/学習院|明治大学|^明大$|青山学院|立教|中央大学|^中大$|法政|関西大学|関西学院|同志社|立命館/.test(e)) return { cog: 12, grit: 6 };
    if (/(東京|京都|大阪|名古屋|東北|九州|北海道|神戸|筑波|横浜国立|千葉|埼玉|一橋|東京工業|東京都立|大阪公立|静岡|新潟|信州|岡山|広島|熊本|金沢|岐阜|三重|滋賀|和歌山|香川|愛媛|長崎|鹿児島|琉球|山口|島根|鳥取|富山|福井|山梨|群馬|宇都宮|茨城|岩手|秋田|山形|福島|弘前|高知|徳島|佐賀|大分|宮崎)大学$/.test(e) && !/学園|学院|工科|経済|産業|商科|女子|国際|文化|芸術|医療|福祉|情報|電機|理科/.test(e)) return { cog: 12, grit: 6 };
    if (/成城|成蹊|明治学院|獨協|國學院|国学院|武蔵大学|東京理科|芝浦工業/.test(e)) return { cog: 9, grit: 5 };
    if (/日本大学|^日大$|東洋大学|^東洋大$|駒澤大学|駒沢大学|専修大学|京都産業|近畿大学|甲南大学|龍谷大学/.test(e)) return { cog: 7, grit: 4 };
    if (/大東文化|東海大学|亜細亜|帝京|国士舘|神奈川大学|工学院|東京電機|拓殖|玉川|東京経済|東京都市|立正|大妻|実践女子|共立女子|昭和女子|東京女子|日本女子|津田塾|学習院女子|桜美林|関東学院|文教|順天堂|明星|武蔵野大学|杏林|東京農業|二松学舎|白百合|聖心|フェリス|関西外|文化学園|流通経済|大正大学|桃山学院|駿河台/.test(e)) return { cog: 5, grit: 3 };
    if (/専門学校|高等学校|高校卒|高卒/.test(e)) return { cog: 3, grit: 2 };
    return { cog: 3, grit: 2 };
}
