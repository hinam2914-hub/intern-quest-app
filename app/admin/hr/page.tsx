...
const HR_METRICS = [
    ...
    { key: "mensdan_kakutei", label: "面談確定数", color: "#6366f1", unit: "件" },
    { key: "mensdan_seiritsu", label: "面談成立数", color: "#8b5cf6", unit: "件" },
    { key: "nyusha", label: "入社数", color: "#34d399", unit: "件" },
    { key: "zanryu", label: "残留数", color: "#06b6d4", unit: "件" },
    { key: "shanai_zentai", label: "社内全体", color: "#f59e0b", unit: "件" },
];
...
export default function HRStatsPage() {
    ...
    const [graphMetric, setGraphMetric] = useState("nyusha");
    ...
    const graphData: MonthData[] = getMonthList().map(ym => {
        ...
        return row;
    }).filter(row => HR_METRICS.some(m => row[`${m.key}_actual`] > 0 || row[`${m.key}_target`] > 0));
    ...
    const selectedMetric = HR_METRICS.find(m => m.key === graphMetric)!;
            ...
            <div style={{ position: "relative", zIndex: 1, maxWidth: 1000, margin: "0 auto" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
                    <div>
                        ...
                        <h1 style={{ fontSize: 28, fontWeight: 800, color: "#f9fafb", margin: "4px 0 0" }}>HR事業部 月次成績</h1>
                    </div>
