...
const IP_METRICS = [
    ...
    { key: "kaiyaku", label: "解約数", color: "#6366f1", unit: "件" },
    { key: "kaijo_rate", label: "解除率", color: "#06b6d4", unit: "%" },
    { key: "kakutoku", label: "獲得数", color: "#34d399", unit: "件" },
];
...
export default function IPStatsPage() {
    ...
    const [graphMetric, setGraphMetric] = useState("kaiyaku");
    ...
    useEffect(() => {
        const load = async () => {
            ...
            const { data: deptRows } = await supabase.from("departments").select("id, name").ilike("name", "%IP%");
    ...
    const graphData: MonthData[] = getMonthList().map(ym => {
        ...
        return row;
    }).filter(row => IP_METRICS.some(m => row[`${m.key}_actual`] > 0 || row[`${m.key}_target`] > 0));
    ...
    const selectedMetric = IP_METRICS.find(m => m.key === graphMetric)!;
            ...
            <div style={{ position: "relative", zIndex: 1, maxWidth: 1000, margin: "0 auto" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
                    <div>
                        ...
                        <h1 style={{ fontSize: 28, fontWeight: 800, color: "#f9fafb", margin: "4px 0 0" }}>IP事業部 月次成績</h1>
                    </div>
