...
const CB_METRICS = [
    ...
    { key: "jyushu", label: "受注数", color: "#34d399" },
    { key: "nohin", label: "納品数", color: "#ec4899" },
];
...
export default function CBStatsPage() {
    ...
    const [graphMetric, setGraphMetric] = useState("appoint");
    ...
    useEffect(() => {
        const load = async () => {
            ...
            const { data: deptRows } = await supabase.from("departments").select("id, name").ilike("name", "%CB%");
    ...
    const graphData: MonthData[] = getMonthList().map(ym => {
        const row: MonthData = { year_month: ym.slice(5) }; // MM表示
        CB_METRICS.forEach(m => {
        ...
        return row;
    }).filter(row => CB_METRICS.some(m => row[`${m.key}_actual`] > 0 || row[`${m.key}_target`] > 0));
    ...
    const selectedMetric = CB_METRICS.find(m => m.key === graphMetric)!;
            ...
            <div style={{ position: "relative", zIndex: 1, maxWidth: 1000, margin: "0 auto" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
                    <div>
                        ...
                        <h1 style={{ fontSize: 28, fontWeight: 800, color: "#f9fafb", margin: "4px 0 0" }}>CB事業部 月次成績</h1>
                    </div>
                                ...
                                <div key={m.key} style={{ padding: "16px", borderRadius: 12, background: "rgba(255,255,255,0.02)", border: `1px solid ${m.color}30` }}>
                                    ...
                                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                                        <div>
                                            ...
                                            <input
                                                type="number"
                                                value={inputs[m.key]?.target || ""}
                                                onChange={(e) => setInputs(prev => ({ ...prev, [m.key]: { ...prev[m.key], target: e.target.value } }))}
                                                placeholder="0"
                                                style={{ width: "100%", padding: "8px 10px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.05)", color: "#f9fafb", fontSize: 14, outline: "none", boxSizing: "border-box" }}
                                            />
                                        </div>
                                        <div>
                                            ...
                                            <input
                                                type="number"
                                                value={inputs[m.key]?.actual || ""}
                                                onChange={(e) => setInputs(prev => ({ ...prev, [m.key]: { ...prev[m.key], actual: e.target.value } }))}
                                                placeholder="0"
                                                style={{ width: "100%", padding: "8px 10px", borderRadius: 8, border: `1px solid ${m.color}40`, background: `${m.color}10`, color: "#f9fafb", fontSize: 14, outline: "none", boxSizing: "border-box" }}
                                            />
                                        </div>
                                    </div>
                                    {target > 0 && (
