...
export default function RecruitPage() {
    ...
    const [myStats, setMyStats] = useState({ dm: 0, mentsuna: 0, interview: 0, hire: 0, points: 0 });
    ...
    const loadAll = useCallback(async (uid: string) => {
        // 自分の集計
        const { data: mine } = await supabase.from("recruit_progress").select("action_type, count, points").eq("user_id", uid);
        const agg = { dm: 0, mentsuna: 0, interview: 0, hire: 0, points: 0 };
        ...
        const { data: all } = await supabase.from("recruit_progress").select("user_id, points");
    ...
    // pt付与共通処理
    const award = async (actionType: string, count: number, points: number, note?: string) => {
        if (busy) return;
        setBusy(true);
        try {
            // recruit_progress に記録
            await supabase.from("recruit_progress").insert([{ user_id: userId, action_type: actionType, count, points, note: note || null }]);
            // user_points 更新（キーは id）
            ...
            // points_history に記録（total_earned/レベルに反映）
    ...
    const recordDM = () => {
        ...
        award("dm", dmInput, units * PT.dm, `${dmInput}通 (LINE報告済み)`);
    };
    ...
    const completeRequest = async (req: Req) => {
        ...
        await award("interview", 1, PT.interview, "募集経由の面談実施");
        setBusy(false);
    };
        ...
        <div style={{ minHeight: "100vh", background: "radial-gradient(ellipse at 30% 0%, #2a1a3e 0%, #0b0b16 55%)", padding: "26px 16px 90px" }}>
            <div style={{ maxWidth: 520, margin: "0 auto" }}>
                ...
                <div style={{ ...card, background: "linear-gradient(150deg, rgba(139,92,246,0.16), rgba(99,102,241,0.05))", border: "1px solid rgba(139,92,246,0.4)" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 14 }}>
                        {[
                            ...
                            { label: "メンつな", value: myStats.mentsuna, unit: "件" },
                ...
                <div style={card}>
                    ...
                    {/* メンつな / 面談 / 入社 */}
                    {[
                        { key: "mentsuna", icon: "🤝", label: "メンつな（面談設定）", pt: PT.mentsuna },
                        ...
                        <div key={a.key} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, padding: "12px", borderRadius: 12, background: a.key === "hire" ? "rgba(252,211,77,0.08)" : "rgba(255,255,255,0.03)", border: a.key === "hire" ? "1px solid rgba(252,211,77,0.3)" : "1px solid rgba(255,255,255,0.06)" }}>
                            ...
                            <button onClick={() => award(a.key, 1, a.pt, "自己申告")} disabled={busy} style={{ padding: "8px 16px", borderRadius: 10, border: "none", background: a.key === "hire" ? "linear-gradient(135deg,#f59e0b,#d97706)" : "linear-gradient(135deg,#8b5cf6,#7c3aed)", color: "#fff", fontWeight: 800, fontSize: 12.5, cursor: "pointer" }}>+{a.pt}</button>
                        </div>
                    ))}
                </div>
